            ...
            if(DB::table('tokendb')->where('token',$tokenz)->count()>0){
                if(DB::table('posts')->where('slno',$postno)->count()>0){
                    ...
                    if($tok_email->user_email == $usrsl_email->email){
                        if(DB::table('post_like')->where('post_slno',$usersl)->where('user_email',$usrsl_email->email)->count()>0){
                            DB::table('post_like')->where('post_slno',$usersl)->where('user_email',$usrsl_email->email)->delete();
                        ...
                        }else if(DB::table('post_dislike')->where('post_slno',$usersl)->where('user_email',$usrsl_email->email)->count()>0){
